 ...
 * Tests run on the upload scheduler so the web and native controls cannot
 * open SMB/TLS transports alongside an upload or each other. */
static esp_err_t upload_test_send(httpd_req_t *req, const char *backend_id)
{
    uint32_t generation = 0;
    esp_err_t err = uploader_test_request(backend_id, &generation);
    ...
    if (err != ESP_OK) {
        ...
        httpd_resp_sendstr(req,
            "{\"accepted\":false,\"message\":\"A connection test is already active\"}");
        return ESP_OK;
    }

    char json[72];
    snprintf(json, sizeof(json),
             "{\"accepted\":true,\"generation\":%lu}",
